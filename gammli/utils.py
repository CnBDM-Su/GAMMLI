...
def get_interaction_list(tr_x, val_x, tr_y, val_y, pred_tr, pred_val, feature_list, feature_type_list, active_main_effect_index, user_feature_list, item_feature_list, task_type="Regression", interaction_restrict=None):
    ...
    '''
    c_types = preprocessor_.col_types_ 
    c_n_bins = preprocessor_.col_n_bins_
...
def plot_trajectory(data_dict_logs, log_scale=True, save_eps=False, save_png=False, folder="./results/", name="trajectory_plot"):
    ...
    t1, t2, t3, t4 , t5= [data_dict_logs["err_train_main_effect_training"], data_dict_logs["err_train_main_effect_tuning"],
                data_dict_logs["err_train_interaction_training"], data_dict_logs["err_train_interaction_tuning"], data_dict_logs["err_train_mf"]]
    v1, v2, v3, v4 , v5= [data_dict_logs["err_val_main_effect_training"], data_dict_logs["err_val_main_effect_tuning"], 
                data_dict_logs["err_val_interaction_training"], data_dict_logs["err_val_interaction_tuning"], data_dict_logs["err_val_mf"]]
    
    offset1x = (0.25 - len(t1) / len(t1 + t2 + t3 + t4 + t5)) * 300
    offset2x = (0.45 - len(t1 + t2) / len(t1 + t2 + t3 + t4 + t5)) * 300
    offset3x = (0.65 - len(t1 + t2 + t3) / len(t1 + t2 + t3 + t4)) * 300
    offset4x = (0.85 - len(t1 + t2 + t3 + t4) / len(t1 + t2 + t3 + t4 + t5)) * 300
    
    fig = plt.figure(figsize=(16, 6))
    ax1 = plt.subplot(1, 2, 1)
    ax1.plot(np.arange(1, len(t1 + t2) + 1, 1), t1 + t2, color="r")
    ax1.plot(np.arange(len(t1 + t2) + 1, len(t1 + t2 + t3 + t4) + 1, 1), t3 + t4, color="b")
    ax1.plot(np.arange(len(t1 + t2 + t3 + t4),len(t1 + t2 + t3 + t4 + t5),1), t5 ,color='g')
    if log_scale:
        ax1.set_yscale("log")
        ax1.set_yticks((10 ** np.linspace(np.log10(np.nanmin(t1 + t2 + t3 + t4 + t5)), np.log10(np.nanmax(t1 + t2 + t3 + t4 + t5)), 6)).round(5))
        ax1.get_yaxis().set_major_formatter(matplotlib.ticker.ScalarFormatter())
        ax1.get_yaxis().set_minor_formatter(matplotlib.ticker.NullFormatter())
        ax1.set_xlabel("Number of Epochs", fontsize=12)
        ax1.set_ylabel("Training Loss (Log Scale)", fontsize=12)
        hp1 = ((np.log10(t1[-1]) - np.log10(np.min(t1 + t2 + t3 + t4 + t5))) 
            / (np.log10(np.max(t1 + t2 + t3 + t4 + t5)) - np.log10(np.min(t1 + t2 + t3 + t4 + t5))))
        hp2 = ((np.log10((t1 + t2)[-1]) - np.log10(np.min(t1 + t2 + t3 + t4 + t5))) 
            / (np.log10(np.max(t1 + t2 + t3 + t4 + t5)) - np.log10(np.min(t1 + t2 + t3 + t4 + t5))))
        hp3 = ((np.log10((t1 + t2 + t3)[-1]) - np.log10(np.min(t1 + t2 + t3 + t4 + t5))) 
            / (np.log10(np.max(t1 + t2 + t3 + t4 + t5)) - np.log10(np.min(t1 + t2 + t3 + t4 + t5))))
        hp4 = ((np.log10((t1 + t2 + t3 + t4)[-1]) - np.log10(np.min(t1 + t2 + t3 + t4 + t5))) 
            / (np.log10(np.max(t1 + t2 + t3 + t4 + t5)) - np.log10(np.min(t1 + t2 + t3 + t4 + t5))))
    else:
        ...
        ax1.set_xlabel("Number of Epochs", fontsize=12)
        ax1.set_ylabel("Training Loss", fontsize=12)
        hp1 = (t1[-1] - np.min(t1 + t2 + t3 + t4 + t5)) / (np.max(t1 + t2 + t3 + t4 + t5) - np.min(t1 + t2 + t3 + t4 + t5))
        hp2 = ((t1 + t2)[-1] - np.min(t1 + t2 + t3 + t4 + t5)) / (np.max(t1 + t2 + t3 + t4 + t5) - np.min(t1 + t2 + t3 + t4 + t5))
        hp3 = ((t1 + t2 + t3)[-1] - np.min(t1 + t2 + t3 + t4 + t5)) / (np.max(t1 + t2 + t3 + t4 + t5) - np.min(t1 + t2 + t3 + t4 + t5))
        hp4 = ((t1 + t2 + t3 + t4)[-1] - np.min(t1 + t2 + t3 + t4 + t5)) / (np.max(t1 + t2 + t3 + t4 + t5) - np.min(t1 + t2 + t3 + t4 + t5))
        
    offset1y = 65 if hp1 < 0.6 else -65
    offset2y = 65 if hp2 < 0.6 else -65
    offset3y = 65 if hp3 < 0.6 else -65
    offset4y = 65 if hp4 < 0.6 else -65
    
    if len(t2) > 0:
        ax1.annotate("Prune \n Main \n Effects", ((len(t1) + 1), t1[-1]), xycoords="data",
    ...
    if len(t4) > 0:
        ax1.annotate("Prune \n Manifest \n Interactions", ((len(t1 + t2 + t3) + 1), (t1 + t2 + t3)[-1]), xycoords="data",
                ...
                horizontalalignment="center", verticalalignment="top")
    if len(t5) > 0:
        ax1.annotate("Add \n Latent \n Interactions", ((len(t1 + t2 + t3 + t4) + 1), (t1 + t2 + t3 + t4)[-1]), xycoords="data",
                xytext=(offset4x, offset4y), textcoords="offset pixels", arrowprops=dict(facecolor="black", shrink=0.1), fontsize=10,
                horizontalalignment="center", verticalalignment="top")
    ...
    ax2.plot(np.arange(len(v1 + v2 + v3 + v4) + 1, len(v1 + v2 + v3 + v4 + v5) + 1, 1), v5, color="g")
    if log_scale:
        ax2.set_yscale("log")
        ax2.set_yticks((10 ** np.linspace(np.log10(np.nanmin(v1 + v2 + v3 + v4 + v5)), np.log10(np.nanmax(v1 + v2 + v3 + v4 + v5)), 6)).round(5))
        ax2.get_yaxis().set_major_formatter(matplotlib.ticker.ScalarFormatter())
        ax2.get_yaxis().set_minor_formatter(matplotlib.ticker.NullFormatter())
        ax2.set_xlabel("Number of Epochs", fontsize=12)
        ax2.set_ylabel("Validation Loss (Log Scale)", fontsize=12)
        hp1 = ((np.log10(v1[-1]) - np.log10(np.min(v1 + v2 + v3 + v4 + v5)))
            / (np.log10(np.max(v1 + v2 + v3 + v4 + v5)) - np.log10(np.min(v1 + v2 + v3 + v4 + v5))))
        hp2 = ((np.log10((v1 + v2)[-1]) - np.log10(np.min(v1 + v2 + v3 + v4 + v5))) 
            / (np.log10(np.max(v1 + v2 + v3 + v4 + v5)) - np.log10(np.min(v1 + v2 + v3 + v4 + v5))))
        hp3 = ((np.log10((v1 + v2 + v3)[-1]) - np.log10(np.min(v1 + v2 + v3 + v4 + v5))) 
            / (np.log10(np.max(v1 + v2 + v3 + v4 + v5)) - np.log10(np.min(v1 + v2 + v3 + v4 + v5))))
    else:
    ...
    offset3y = 65 if hp3 < 0.6 else -65
    if len(v2) > 0:
        ax2.annotate("Prune \n Main \n Effects", ((len(v1) + 1), v1[-1]), xycoords="data",
                xytext=(offset1x, offset1y), textcoords="offset pixels", arrowprops=dict(facecolor="black", shrink=0.1), fontsize=10,
                horizontalalignment="center", verticalalignment="top")
    if len(v3) > 0:
    ...
    if len(v4) > 0:
        ax2.annotate("Prune \n Manifest \n Interactions", ((len(v1 + v2 + v3) + 1), (v1 + v2 + v3)[-1]), xycoords="data",
                ...
                horizontalalignment="center", verticalalignment="top")
    if len(v5) > 0:
        ax2.annotate("Add \n Latent \n Interactions", ((len(v1 + v2 + v3 + v4) + 1), (v1 + v2 + v3 + v4)[-1]), xycoords="data",
                xytext=(offset4x, offset4y), textcoords="offset pixels", arrowprops=dict(facecolor="black", shrink=0.1), fontsize=10,
                horizontalalignment="center", verticalalignment="top")
...
def global_visualize_density(data_dict_global, main_effect_num=10**5, interaction_num=10**5, cols_per_row=4,
    ...
    fig.suptitle('Main Effects & Manifest Interactions',fontsize=18)
