...
class OutputLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, input_num, interact_num,task_type='Regression',multi_type_num=5):
        ...
        if self.task_type != "MultiClassification":
            ...
            if self.task_type == "Ordinal_Regression":
                ...
                self.ordinal_bias = self.add_weight(name="ordinal_bias",
                                           shape=[1,self.multi_type_num-1],
                                           initializer=tf.zeros_initializer(),
                                           trainable=True)
        ...
        if self.task_type == "MultiClassification":
            ...
            self.multi_weights = self.add_weight(name="multi_weights",
                                              shape=[1,self.multi_type_num],
                                              initializer=tf.keras.initializers.Orthogonal(),
                                              trainable=True)
            self.multi_bias = self.add_weight(name="numti_bias",
                                           shape=[self.multi_type_num],
                                           initializer=tf.zeros_initializer(),
                                           trainable=True) 
            '''
            self.main_effect_weights = self.add_weight(name="multi_output_weights",
                                              shape=[self.input_num, self.multi_type_num],
                                              initializer=tf.keras.initializers.GlorotNormal(),
                                              trainable=True)
            
            self.main_effect_switcher = self.add_weight(name="multi_subnet_switcher",
                                              shape=[self.input_num, self.multi_type_num],
                                              initializer=tf.ones_initializer(),
                                              trainable=False)
            self.interaction_weights = self.add_weight(name="multi_interaction_weights",
                                              shape=[self.interact_num, self.multi_type_num],
                                              initializer=tf.keras.initializers.Orthogonal(),
                                              trainable=True)
            self.interaction_switcher = self.add_weight(name="multi_interaction_switcher",
                                              shape=[self.interact_num, self.multi_type_num],
                                              initializer=tf.ones_initializer(),
                                              trainable=False)
            self.main_effect_output_bias = self.add_weight(name="multi_main_effect_output_bias",
                                           shape=[self.multi_type_num],
                                           initializer=tf.zeros_initializer(),
                                           trainable=True)
    ...
    def call(self, inputs):
        ...
        if self.interact_num > 0:
            ...
            if self.task_type == "Ordinal_Regression":
                output = self.ordinal_bias - tf.matmul(self.input_main_effect, self.main_effect_switcher * self.main_effect_weights)
